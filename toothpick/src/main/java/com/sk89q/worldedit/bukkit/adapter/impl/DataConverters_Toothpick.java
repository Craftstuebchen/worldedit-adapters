...
class DataConverters_Toothpick extends DataFixerBuilder implements com.sk89q.worldedit.world.DataFixer {

    @SuppressWarnings("unchecked")
    @Override
    public <T> T fixUp(FixType<T> type, T original, int srcVer) {
        if (type == FixTypes.CHUNK) {
            return (T) fixChunk((BinaryTag) original, srcVer);
        } else if (type == FixTypes.BLOCK_ENTITY) {
            return (T) fixBlockEntity((BinaryTag) original, srcVer);
        } else if (type == FixTypes.ENTITY) {
            return (T) fixEntity((BinaryTag) original, srcVer);
    ...
    private BinaryTag fixChunk(BinaryTag originalChunk, int srcVer) {
        ...
        return adapter.toNative(fixed);
    }

    private BinaryTag fixBlockEntity(BinaryTag origTileEnt, int srcVer) {
    ...
    private BinaryTag fixEntity(BinaryTag origEnt, int srcVer) {
    ...
    private String fixBlockState(String blockState, int srcVer) {
        ...
        net.minecraft.nbt.CompoundTag fixed = (net.minecraft.nbt.CompoundTag) INSTANCE.fixer.update(net.minecraft.util.datafix.fixes.References.BLOCK_STATE, dynamic, srcVer, DATA_VERSION).getValue();
    ...
    private String fixBiome(String key, int srcVer) {
        return fixName(key, srcVer, net.minecraft.util.datafix.fixes.References.BIOME);
    }

    private String fixItemType(String key, int srcVer) {
        return fixName(key, srcVer, net.minecraft.util.datafix.fixes.References.ITEM_NAME);
    }
    ...
    private final Spigot_Toothpick adapter;

    private static final net.minecraft.nbt.NbtOps OPS_NBT = NbtOps.INSTANCE;
    private static final int LEGACY_VERSION = 1343;
    private static int DATA_VERSION;
    static DataConverters_Toothpick INSTANCE;
    ...
    public enum LegacyType {
        ...
        CHUNK(net.minecraft.util.datafix.fixes.References.CHUNK),
        BLOCK_ENTITY(net.minecraft.util.datafix.fixes.References.BLOCK_ENTITY),
        ENTITY(net.minecraft.util.datafix.fixes.References.ENTITY),
        ITEM_INSTANCE(net.minecraft.util.datafix.fixes.References.ITEM_STACK),
        OPTIONS(DataFixTypes.OPTIONS.getType()),
        STRUCTURE(DataFixTypes.STRUCTURE.getType());

        private final DSL.TypeReference type;

        LegacyType(DSL.TypeReference type) {
            this.type = type;
    ...
    DataConverters_Toothpick(int dataVersion, Spigot_Toothpick adapter) {
        super(dataVersion);
    ...
    public DataFixer build(final Executor executor) {
        return this.fixer = new WrappedDataFixer(net.minecraft.util.datafix.DataFixers.getDataFixer());
    }
    ...
    private class WrappedDataFixer implements DataFixer {
        ...
        private net.minecraft.nbt.CompoundTag convert(LegacyType type, net.minecraft.nbt.CompoundTag cmp, int sourceVer, int desiredVersion) {
            List<DataConverter> converters = DataConverters_Toothpick.this.converters.get(type);
            ...
            List<DataInspector> inspectors = DataConverters_Toothpick.this.inspectors.get(type);
    ...
    public static net.minecraft.nbt.CompoundTag convert(DSL.TypeReference type, net.minecraft.nbt.CompoundTag cmp) {
        int i = cmp.contains("DataVersion", 99) ? cmp.getInt("DataVersion") : -1;
        return convert(type, cmp, i);
    }

    public static net.minecraft.nbt.CompoundTag convert(DSL.TypeReference type, net.minecraft.nbt.CompoundTag cmp, int sourceVer) {
        return convert(type, cmp, sourceVer, DATA_VERSION);
    }

    public static net.minecraft.nbt.CompoundTag convert(DSL.TypeReference type, net.minecraft.nbt.CompoundTag cmp, int sourceVer, int targetVer) {
    ...
    private static void convertItem(net.minecraft.nbt.CompoundTag compoundTag, String key, int sourceVer, int targetVer) {
        if (compoundTag.contains(key, 10)) {
            convertCompound(LegacyType.ITEM_INSTANCE, compoundTag, key, sourceVer, targetVer);
        }
    }

    private static void convertItems(net.minecraft.nbt.CompoundTag compoundTag, String key, int sourceVer, int targetVer) {
        if (compoundTag.contains(key, 9)) {
            ListTag nbttaglist = compoundTag.getList(key, 10);
    ...
    private static class DataInspectorBlockEntity implements DataInspector {
        ...
        public net.minecraft.nbt.CompoundTag inspect(net.minecraft.nbt.CompoundTag cmp, int sourceVer, int targetVer) {
            if (!cmp.contains("tag", 10)) {
                ...
                net.minecraft.nbt.CompoundTag compoundTag1 = cmp.getCompound("tag");

                if (compoundTag1.contains("BlockEntityTag", 10)) {
                    net.minecraft.nbt.CompoundTag compoundTag2 = compoundTag1.getCompound("BlockEntityTag");
                    ...
                    if (s1 == null) {
                        ...
                        flag = !compoundTag2.contains("id");
                        compoundTag2.putString("id", s1);
                    }

                    convert(LegacyType.BLOCK_ENTITY, compoundTag2, sourceVer, targetVer);
                    if (flag) {
    ...
    private static class DataInspectorEntity implements DataInspector {
        ...
        private static final Logger a = LogManager.getLogger(DataConverters_Toothpick.class);
        ...
        public net.minecraft.nbt.CompoundTag inspect(net.minecraft.nbt.CompoundTag cmp, int sourceVer, int targetVer) {
            ...
            if (compoundTag1.contains("EntityTag", 10)) {
                net.minecraft.nbt.CompoundTag compoundTag2 = compoundTag1.getCompound("EntityTag");
                String s = cmp.getString("id");
                String s1;
                ...
                if (s1 == null) {
                    ...
                    flag = !compoundTag2.contains("id", 8);
                    compoundTag2.putString("id", s1);
                }

                convert(LegacyType.ENTITY, compoundTag2, sourceVer, targetVer);
                if (flag) {
                    compoundTag2.remove("id");
                }
            }
    ...
    private abstract static class DataInspectorTagged implements DataInspector {
        ...
        abstract net.minecraft.nbt.CompoundTag inspectChecked(net.minecraft.nbt.CompoundTag compoundTag, int sourceVer, int targetVer);
    }
    ...
    private static class DataInspectorItemList extends DataInspectorTagged {
        ...
        net.minecraft.nbt.CompoundTag inspectChecked(net.minecraft.nbt.CompoundTag compoundTag, int sourceVer, int targetVer) {
            for (String s : this.keys) {
                DataConverters_Toothpick.convertItems(compoundTag, s, sourceVer, targetVer);
            }
    ...
    private static class DataInspectorItem extends DataInspectorTagged {
        ...
        net.minecraft.nbt.CompoundTag inspectChecked(net.minecraft.nbt.CompoundTag compoundTag, int sourceVer, int targetVer) {
            for (String key : this.keys) {
                DataConverters_Toothpick.convertItem(compoundTag, key, sourceVer, targetVer);
            }
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            if ("minecraft:banner".equals(cmp.getString("id")) && cmp.contains("tag", 10)) {
                ...
                if (compoundTag1.contains("BlockEntityTag", 10)) {
                    ...
                    if (compoundTag2.contains("Base", 99)) {
                        cmp.putShort("Damage", (short) (compoundTag2.getShort("Base") & 15));
                        if (compoundTag1.contains("display", 10)) {
                            net.minecraft.nbt.CompoundTag compoundTag3 = compoundTag1.getCompound("display");

                            if (compoundTag3.contains("Lore", 9)) {
                                ListTag nbttaglist = compoundTag3.getList("Lore", 8);
                        ...
                        compoundTag2.remove("Base");
                        if (compoundTag2.isEmpty()) {
                            compoundTag1.remove("BlockEntityTag");
                        }
    ...
    private static class DataConverterPotionId implements DataConverter {
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            if ("minecraft:potion".equals(cmp.getString("id"))) {
                ...
                if (!compoundTag1.contains("Potion", 8)) {
                    String s = DataConverterPotionId.potions[short0 & 127];

                    compoundTag1.putString("Potion", s == null ? "minecraft:water" : s);
                    cmp.put("tag", compoundTag1);
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            if ("minecraft:spawn_egg".equals(cmp.getString("id"))) {
                ...
                if (!compoundTag2.contains("id", 8)) {
                    ...
                    if (s != null) {
                        compoundTag2.putString("id", s);
                        compoundTag1.put("EntityTag", compoundTag2);
                        cmp.put("tag", compoundTag1);
                    }
                }
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            if (!"MobSpawner".equals(cmp.getString("id"))) {
                ...
                if (cmp.contains("EntityId", 8)) {
                    ...
                    net.minecraft.nbt.CompoundTag compoundTag1 = cmp.getCompound("SpawnData");

                    compoundTag1.putString("id", s.isEmpty() ? "Pig" : s);
                    cmp.put("SpawnData", compoundTag1);
                    cmp.remove("EntityId");
                }
                ...
                if (cmp.contains("SpawnPotentials", 9)) {
                    ...
                    for (int i = 0; i < nbttaglist.size(); ++i) {
                        net.minecraft.nbt.CompoundTag compoundTag2 = nbttaglist.getCompound(i);

                        if (compoundTag2.contains("Type", 8)) {
                            net.minecraft.nbt.CompoundTag compoundTag3 = compoundTag2.getCompound("Properties");

                            compoundTag3.putString("id", compoundTag2.getString("Type"));
                            compoundTag2.put("Entity", compoundTag3);
                            compoundTag2.remove("Type");
                            compoundTag2.remove("Properties");
                        }
                    }
                }
    ...
    private static class DataConverterSaddle implements DataConverter {
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            if ("EntityHorse".equals(cmp.getString("id")) && !cmp.contains("SaddleItem", 10) && cmp.getBoolean("Saddle")) {
                net.minecraft.nbt.CompoundTag compoundTag1 = new net.minecraft.nbt.CompoundTag();

                compoundTag1.putString("id", "minecraft:saddle");
                compoundTag1.putByte("Count", (byte) 1);
                compoundTag1.putShort("Damage", (short) 0);
                cmp.put("SaddleItem", compoundTag1);
                cmp.remove("Saddle");
            }
    ...
    private static class DataConverterHanging implements DataConverter {
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            ...
            if ((flag || flag1) && !cmp.contains("Facing", 99)) {
                net.minecraft.core.Direction enumdirection;

                if (cmp.contains("Direction", 99)) {
                    enumdirection = net.minecraft.core.Direction.from2DDataValue(cmp.getByte("Direction"));
                    ...
                    enumdirection = net.minecraft.core.Direction.from2DDataValue(cmp.getByte("Dir"));
                    cmp.remove("Dir");
                }
    ...
    private static class DataConverterRiding implements DataConverter {
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            while (cmp.contains("Riding", 10)) {
                net.minecraft.nbt.CompoundTag compoundTag1 = this.b(cmp);

                this.convert(cmp, compoundTag1);
                cmp = compoundTag1;
            }
        ...
        protected void convert(net.minecraft.nbt.CompoundTag compoundTag, net.minecraft.nbt.CompoundTag compoundTag1) {
            ListTag nbttaglist = new ListTag();

            nbttaglist.add(compoundTag);
            compoundTag1.put("Passengers", nbttaglist);
        }

        protected net.minecraft.nbt.CompoundTag b(net.minecraft.nbt.CompoundTag compoundTag) {
            net.minecraft.nbt.CompoundTag compoundTag1 = compoundTag.getCompound("Riding");

            compoundTag.remove("Riding");
            return compoundTag1;
        }
    }
    ...
    private static class DataConverterBook implements DataConverter {
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            if ("minecraft:written_book".equals(cmp.getString("id"))) {
                ...
                if (compoundTag1.contains("pages", 9)) {
                    ListTag nbttaglist = compoundTag1.getList("pages", 8);
                    ...
                    for (int i = 0; i < nbttaglist.size(); ++i) {
                        ...
                        if (!"null".equals(s) && !StringUtil.isNullOrEmpty(s)) {
                            if ((s.charAt(0) != 34 || s.charAt(s.length() - 1) != 34) && (s.charAt(0) != 123 || s.charAt(s.length() - 1) != 125)) {
                                ...
                                try {
                                    object = net.minecraft.util.GsonHelper.fromJson(DataConverterSignText.a, s, Component.class, true);
                                    if (object == null) {
                                ...
                                if (object == null) {
                                    try {
                                        object = Component.ChatSerializer.a(s);
                                ...
                                if (object == null) {
                                    try {
                                        object = Component.ChatSerializer.b(s);
                        ...
                        nbttaglist.add(i, StringTag.valueOf(Component.ChatSerializer.a((Component) object)));
                    }

                    compoundTag1.put("pages", nbttaglist);
                }
            }
    ...
    private static class DataConverterPotionWater implements DataConverter {
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            ...
            if ("minecraft:potion".equals(s) || "minecraft:splash_potion".equals(s) || "minecraft:lingering_potion".equals(s) || "minecraft:tipped_arrow".equals(s)) {
                ...
                if (!compoundTag1.contains("Potion", 8)) {
                    compoundTag1.putString("Potion", "minecraft:water");
                }
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            if ("minecraft:shulker_box".equals(cmp.getString("id")) && cmp.contains("tag", 10)) {
                ...
                if (compoundTag1.contains("BlockEntityTag", 10)) {
                    ...
                    if (compoundTag2.getList("Items", 10).isEmpty()) {
                        compoundTag2.remove("Items");
                    }

                    int i = compoundTag2.getInt("Color");
    ...
    private static class DataConverterBedBlock implements DataConverter {
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            ...
            try {
                ...
                int i = compoundTag1.getInt("xPos");
                int j = compoundTag1.getInt("zPos");
                ListTag nbttaglist = compoundTag1.getList("TileEntities", 10);
                ListTag nbttaglist1 = compoundTag1.getList("Sections", 10);

                for (int k = 0; k < nbttaglist1.size(); ++k) {
                    net.minecraft.nbt.CompoundTag compoundTag2 = nbttaglist1.getCompound(k);
                    byte b0 = compoundTag2.getByte("Y");
                    byte[] abyte = compoundTag2.getByteArray("Blocks");
                    ...
                    for (int l = 0; l < abyte.length; ++l) {
                        if (416 == (abyte[l] & 255) << 4) {
                            ...
                            compoundTag3.putString("id", "bed");
                            compoundTag3.putInt("x", i1 + (i << 4));
                            compoundTag3.putInt("y", j1 + (b0 << 4));
                            compoundTag3.putInt("z", k1 + (j << 4));
                            nbttaglist.add(compoundTag3);
                        }
                    }
                }
    ...
    private static class DataConverterBedItem implements DataConverter {
        ...
        public net.minecraft.nbt.CompoundTag convert(net.minecraft.nbt.CompoundTag cmp) {
            if ("minecraft:bed".equals(cmp.getString("id")) && cmp.getShort("Damage") == 0) {
                cmp.putShort("Damage", (short) net.minecraft.world.item.DyeColor.RED.getId());
            }
    ...
    private static class DataConverterSignText implements DataConverter {
            ...
            MutableComponent a(JsonElement jsonelement, Type type, JsonDeserializationContext jsondeserializationcontext) throws JsonParseException {
                if (jsonelement.isJsonPrimitive()) {
                    ...
                    for (JsonElement jsonelement1 : jsonarray) {
                        MutableComponent ichatbasecomponent1 = this
                            .a(jsonelement1, jsonelement1.getClass(), jsondeserializationcontext);
        ...
        private void convert(net.minecraft.nbt.CompoundTag compoundTag, String s) {
            String s1 = compoundTag.getString(s);
            Object object = null;

            if (!"null".equals(s1) && !net.minecraft.util.StringUtil.isNullOrEmpty(s1)) {
                if ((s1.charAt(0) != 34 || s1.charAt(s1.length() - 1) != 34) && (s1.charAt(0) != 123 || s1.charAt(s1.length() - 1) != 125)) {
                    object = new TextComponent(s1);
                } else {
                    try {
                        object = net.minecraft.util.GsonHelper.fromJson(DataConverterSignText.a, s1, Component.class, true);
                        if (object == null) {
                            object = new TextComponent("");
                        }
                    } catch (JsonParseException jsonparseexception) {
                    ...
                    if (object == null) {
                        try {
                            object = Component.ChatSerializer.a(s1);
                        } catch (JsonParseException jsonparseexception1) {
                        }
                    }
                    ...
                    if (object == null) {
                        try {
                            object = Component.ChatSerializer.b(s1);
                        } catch (JsonParseException jsonparseexception2) {
                        }
                    }
            ...
            compoundTag.putString(s, Component.ChatSerializer.a((Component) object));
        }
    }
    ...
    private static class DataInspectorPlayerVehicle implements DataInspector {
        ...
        public net.minecraft.nbt.CompoundTag inspect(net.minecraft.nbt.CompoundTag cmp, int sourceVer, int targetVer) {
            if (cmp.contains("RootVehicle", 10)) {
                net.minecraft.nbt.CompoundTag compoundTag1 = cmp.getCompound("RootVehicle");

                if (compoundTag1.contains("Entity", 10)) {
                    convertCompound(LegacyType.ENTITY, compoundTag1, "Entity", sourceVer, targetVer);
                }
            }
    ...
    private static class DataInspectorStructure implements DataInspector {
        ...
        public net.minecraft.nbt.CompoundTag inspect(net.minecraft.nbt.CompoundTag cmp, int sourceVer, int targetVer) {
            ListTag nbttaglist;
            int j;
            net.minecraft.nbt.CompoundTag compoundTag1;

            if (cmp.contains("entities", 9)) {
                nbttaglist = cmp.getList("entities", 10);

                for (j = 0; j < nbttaglist.size(); ++j) {
                    compoundTag1 = (net.minecraft.nbt.CompoundTag) nbttaglist.get(j);
                    if (compoundTag1.contains("nbt", 10)) {
                        convertCompound(LegacyType.ENTITY, compoundTag1, "nbt", sourceVer, targetVer);
                    }
                }
            }
            ...
            if (cmp.contains("blocks", 9)) {
                ...
                for (j = 0; j < nbttaglist.size(); ++j) {
                    compoundTag1 = (net.minecraft.nbt.CompoundTag) nbttaglist.get(j);
                    if (compoundTag1.contains("nbt", 10)) {
                        convertCompound(LegacyType.BLOCK_ENTITY, compoundTag1, "nbt", sourceVer, targetVer);
                    }
                }
            }
    ...
    private static class DataInspectorChunks implements DataInspector {
        ...
        public net.minecraft.nbt.CompoundTag inspect(net.minecraft.nbt.CompoundTag cmp, int sourceVer, int targetVer) {
            if (cmp.contains("Level", 10)) {
                ...
                if (compoundTag1.contains("Entities", 9)) {
                    nbttaglist = compoundTag1.getList("Entities", 10);

                    for (j = 0; j < nbttaglist.size(); ++j) {
                        nbttaglist.add(j, convert(LegacyType.ENTITY, (net.minecraft.nbt.CompoundTag) nbttaglist.get(j), sourceVer, targetVer));
                    }
                }

                if (compoundTag1.contains("TileEntities", 9)) {
                    nbttaglist = compoundTag1.getList("TileEntities", 10);

                    for (j = 0; j < nbttaglist.size(); ++j) {
                        nbttaglist.add(j, convert(LegacyType.BLOCK_ENTITY, (net.minecraft.nbt.CompoundTag) nbttaglist.get(j), sourceVer, targetVer));
                    }
                }
            }
    ...
    private static class DataInspectorEntityPassengers implements DataInspector {
        ...
        public net.minecraft.nbt.CompoundTag inspect(net.minecraft.nbt.CompoundTag cmp, int sourceVer, int targetVer) {
            if (cmp.contains("Passengers", 9)) {
                ...
                for (int j = 0; j < nbttaglist.size(); ++j) {
                    nbttaglist.add(j, convert(LegacyType.ENTITY, nbttaglist.getCompound(j), sourceVer, targetVer));
                }
            }
    ...
    private static class DataInspectorVillagers implements DataInspector {
        ...
        public net.minecraft.nbt.CompoundTag inspect(net.minecraft.nbt.CompoundTag cmp, int sourceVer, int targetVer) {
            if (entityVillager.equals(new ResourceLocation(cmp.getString("id"))) && cmp.contains("Offers", 10)) {
                net.minecraft.nbt.CompoundTag compoundTag1 = cmp.getCompound("Offers");

                if (compoundTag1.contains("Recipes", 9)) {
                    ListTag nbttaglist = compoundTag1.getList("Recipes", 10);

                    for (int j = 0; j < nbttaglist.size(); ++j) {
                        net.minecraft.nbt.CompoundTag compoundTag2 = nbttaglist.getCompound(j);

                        convertItem(compoundTag2, "buy", sourceVer, targetVer);
                        convertItem(compoundTag2, "buyB", sourceVer, targetVer);
                        convertItem(compoundTag2, "sell", sourceVer, targetVer);
                        nbttaglist.add(j, compoundTag2);
                    }
                }
            }
        ...
        public net.minecraft.nbt.CompoundTag inspect(net.minecraft.nbt.CompoundTag cmp, int sourceVer, int targetVer) {
            if (tileEntityMobSpawner.equals(new ResourceLocation(cmp.getString("id")))) {
                if (cmp.contains("SpawnPotentials", 9)) {
                    ...
                    for (int j = 0; j < nbttaglist.size(); ++j) {
                        net.minecraft.nbt.CompoundTag compoundTag1 = nbttaglist.getCompound(j);
